...
export function compareMembers(
  ...
  notesMap: Map<string, Map<string, Note>>,
  ...
  switch (column) {
    ...
    case 'checkin': {
      const checkedA = notesMap.get(a.memberId)?.get('general notes')?.checkin_1
        ? 1
        : 0
      const checkedB = notesMap.get(b.memberId)?.get('general notes')?.checkin_1
        ? 1
        : 0
